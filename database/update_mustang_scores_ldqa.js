...
// Requires DATABASE_URL env var pointing to LDQA branch
// Usage: DATABASE_URL="postgresql://..." node database/update_mustang_scores_ldqa.js
const dbUrl = process.env.DATABASE_URL;
if (!dbUrl) {
  console.error("ERROR: DATABASE_URL env var is required (point it at the LDQA branch).");
  console.error('Usage: DATABASE_URL="postgresql://..." node database/update_mustang_scores_ldqa.js');
  process.exit(1);
}
...
// Mustang team names — used to auto-detect the season
const MUSTANG_TEAMS = ["Pirates", "Rays", "Orioles", "Astros", "White Sox", "Braves",
  "Yankees", "Marlins", "Nationals", "Padres", "Brewers", "Cubs", "Giants", "Blue Jays",
  "Mariners", "Dodgers", "Red Sox", "Rockies", "Athletics", "Angels"];

(async () => {
  try {
    // 1. Auto-detect Mustang season by finding the season containing these teams
    const seasonRows = await sql`
      ...
      WHERE EXISTS (
        ...
        WHERE st.season_id = s.id AND t.name = 'Pirates'
      )
      AND EXISTS (
        SELECT 1 FROM season_teams st
        JOIN teams t ON t.teamid = st.team_id
        WHERE st.season_id = s.id AND t.name = 'Rockies'
      )
      AND EXISTS (
        SELECT 1 FROM season_teams st
        JOIN teams t ON t.teamid = st.team_id
        WHERE st.season_id = s.id AND t.name = 'Mariners'
      )
    `;
    if (seasonRows.length === 0) {
      console.error("ERROR: Could not find Mustang season (no season with Pirates + Rockies + Mariners).");
      process.exit(1);
    }
    if (seasonRows.length > 1) {
      console.error("ERROR: Multiple matching seasons found:", seasonRows.map(r => `${r.id}: ${r.name}`).join(", "));
      process.exit(1);
    }
    const seasonId = seasonRows[0].id;
    console.log(`Found Mustang season: id=${seasonId} (${seasonRows[0].name})\n`);

    // 2. Look up location_id for "Mission Sports Park"
    ...
    // 3. Get all games for this season with team names
    const games = await sql`
      ...
      WHERE g.season_id = ${seasonId} AND g.game_type = 'regular'
    `;

    console.log(`Found ${games.length} existing games in season ${seasonId}\n`);

    // 4. Handle rescheduled Week 1 game: Rockies vs Giants 2/21 -> 4/28
    //    On dev this game stayed on 2/21 with status=5 while others got new entries.
    //    Check if same situation exists here.
    const reschedCheck = await sql`
      SELECT g.id, g.gamedate, g.gamestatusid
      FROM season_games g
      LEFT JOIN teams ht ON ht.teamid = g.home
      LEFT JOIN teams at ON at.teamid = g.away
      WHERE g.season_id = ${seasonId} AND g.game_type = 'regular'
        AND ht.name = 'Rockies' AND at.name = 'Giants'
        AND g.gamedate = '2026-02-21'
    `;
    if (reschedCheck.length > 0) {
      const gid = reschedCheck[0].id;
      await sql`
        UPDATE season_games
        SET gamedate = '2026-04-28', gametime = '17:00',
            gamestatusid = 1
        WHERE id = ${gid}
      `;
      console.log(`RESCHEDULE: Moved Rockies vs Giants from 2/21 to 4/28 (game #${gid})\n`);

      // Refresh games list after date change
      ...
      `;
      games.length = 0;
      games.push(...refreshed);
    }

    // 5. Build lookup: "date|home|away" -> game row
    const lookup = {};
