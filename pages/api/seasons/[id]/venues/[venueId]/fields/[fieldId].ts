// pages/api/seasons/[id]/venues/[venueId]/fields/[fieldId].ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (![seasonId, venueId, fieldId].every(Number.isFinite)) {
  ...
  try {
    ...
    const { rows: own } = await client.query(
      `SELECT 1
       FROM season_venue_fields f
       JOIN season_venues v ON v.id = f.season_venue_id
       WHERE f.id = $1 AND v.id = $2 AND v.season_id = $3`,
      [fieldId, venueId, seasonId],
    );
      ...
      if (typeof body.name === "string") {
        ...
        const { rows: dup } = await client.query(
          `SELECT id FROM season_venue_fields
            WHERE season_venue_id = $1 AND LOWER(name) = LOWER($2) AND id <> $3 LIMIT 1`,
          [venueId, t, fieldId],
        );
    ...
    if (req.method === "DELETE") {
      await client.query(`DELETE FROM season_venue_fields WHERE id = $1`, [fieldId]);
