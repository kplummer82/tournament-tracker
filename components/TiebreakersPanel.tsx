...
/* ---------- Direction badge — fixed light-mode contrast ---------- */
...
function ActiveTiebreakerRow({
  ...
  const row = (
    ...
    >
      ...
      <span className="flex-1 min-w-0 font-medium text-sm text-foreground truncate">{tb.displayName || tb.code}</span>
...
/* ---------- Clickable row for available (not-yet-added) tiebreakers — matches active row style ---------- */
function AvailableChip({ tb, onClick }: { tb: Tiebreaker; onClick: () => void }) {
  const content = (
    ...
    >
      ...
      <span className="flex-1 min-w-0 font-medium text-sm truncate">{tb.displayName || tb.code}</span>
